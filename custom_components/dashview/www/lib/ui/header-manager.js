...
export class HeaderManager {
    ...
    // A central method to update all parts of the header.
    updateAll() {
      ...
      Object.entries(roomsByFloor).forEach(([floorKey, floorRooms]) => {
        ...
        if (activeMotionRooms.length > 0) {
          ...
          activeMotionRooms.forEach(room => {
            buttonsHTML += `<button class="header-room-button" data-room="${room.key}" data-floor="${floorKey}" data-navigation="#${room.key}" title="${room.config.friendly_name}"><i class="mdi ${this._panel._processIconName(room.config.icon)}"></i></button>`;
          });
        }
      });
